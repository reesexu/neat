...
const moveRunTime = () => gulp.src(`${src}/lib/${regeneratorRuntimeFolderName}/*.js`)
  .pipe(gulp.dest(`${dist}/${miniprogramNpmFolderName}/${regeneratorRuntimeFolderName}`))

// 给指定的js文件头部注入引入regeneratorRuntime的语句，因为需要使用到async语法的都需要引入该文件
const injectRuntime = () => gulp.src([`${dist}/app.js`, `${dist}/{components,models,pages,app.js}/**/*.js`])
...
const withAutoPreview = task => isAutoPreview ? series(task, autoPreview) : series(task) // 这个自动预览暂时有页面不存在的bug
